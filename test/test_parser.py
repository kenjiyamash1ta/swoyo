...
def test_invalid_sender():
    ...
    test_args = [
        "program_name", "-s", "123456789", "-r", "89123456789", "-m", "Hello"
    ]
    with patch.object(sys, "argv", test_args):
        with pytest.raises(
            ValueError, match="Номер отправителя должен быть российским"
        ):
            parse()
...
def test_invalid_recipient():
    ...
    test_args = [
        "program_name", "-s", "+79123456789", "-r", "123456789", "-m", "Hello"
    ]
    with patch.object(sys, "argv", test_args):
        with pytest.raises(
                ValueError, match="Номер получателя должен быть российским"
                ):
            parse()
